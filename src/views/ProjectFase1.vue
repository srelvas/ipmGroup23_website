...
<template>
  <div class="bg-white">
    <div class="max-w-2xl mx-auto py-16 px-4 sm:py-24 sm:px-6 lg:max-w-7xl lg:px-8">
      <h2 class="text-2xl font-extrabold tracking-tight text-gray-900">Evolution and selection of app colour scheme</h2>
      <h3 class="text-xl font-extrabold tracking-tight text-gray-500">This shows the evolution of the colour scheme from left to right</h3>
      <div class="mt-10 grid grid-cols-1 gap-y-10 gap-x-6 sm:grid-cols-2 lg:grid-cols-4 xl:gap-x-8 flex items-center">
        <button @click="productClicked(product)" v-for="product in productsColorScheme" :key="product.id" class="group relative">
          <div class="w-full min-h-80 bg-gray-200 aspect-w-1 aspect-h-1 rounded-md overflow-hidden group-hover:opacity-75 lg:aspect-none">
            <img :src="require(`../assets/${product.imageSrc}`)" :alt="product.imageAlt" class="w-full h-full object-center object-cover lg:w-full lg:h-full" />
          </div>
          <br>
            ...
            <div class="flex min-h-screen text-center md:block md:px-2 lg:px-4" style="font-size: 0;">
                ...
                <div class="flex text-base text-left transform transition w-full md:inline-block md:max-w-2xl md:px-4 md:my-8 md:align-middle lg:max-w-4xl">
                    ...
                    <div class="w-full grid grid-cols-1 gap-y-8 gap-x-6 items-start sm:grid-cols-12 lg:gap-x-8">
                        ...
                        <img :src="require(`../assets/${productChosen.imageSrc}`)" :alt="productChosen.imageAlt" class="object-center object-cover" />
                        </div>
                        <div class="sm:col-span-8 lg:col-span-7">
                            ...
                            <section aria-labelledby="information-heading" class="mt-2">
                                ...
                                <img :src="require(`../assets/${productChosen.imagePalette}`)"/>
                                ...
                                <div class="flex items-center">
                                    <div class="flex items-center">
                                    <StarIcon v-for="rating in [0, 1, 2, 3, 4]" :key="rating" :class="[productChosen.rating > rating ? 'text-gray-900' : 'text-gray-200', 'h-5 w-5 flex-shrink-0']" aria-hidden="true" />
                                    </div>
                                    <p class="sr-only">{{ productChosen.rating }} out of 5 stars</p>
                                    <a href="#" class="ml-3 text-sm font-medium text-indigo-600 hover:text-indigo-500">{{ productChosen.reviewCount }} reviews</a>
                                </div> -->
                                </div>
                            </section>
    ...
    <!-- <div x-ref="handle" @mouseenter="openSideMenuFunction()" class="sr-only sm:not-sr-only sm:border-l sm:bg-gray-100 sm:absolute sm:left-0 sm:inset-y-0 sm:flex sm:items-center sm:w-4 cursor-[ew-resize]" style="touch-action: none">
    ...
    <TransitionRoot as="template" :show="openSideMenu">
        <Dialog as="div" class="fixed inset-0 overflow-hidden" @close="openSideMenu = false">
            ...
            <TransitionChild as="template" enter="transform transition ease-in-out duration-500 sm:duration-700" enter-from="translate-x-full" enter-to="translate-x-0" leave="transform transition ease-in-out duration-500 sm:duration-700" leave-from="translate-x-0" leave-to="translate-x-full">
                ...
                <div class="h-full flex flex-col py-6 bg-white shadow-xl overflow-y-scroll">
                    ...
                    <div class="flex items-start justify-between">
                        ...
                        <button type="button" class="bg-white rounded-md text-gray-400 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500" @click="openSideMenu = false">
